...
The primary purpose of the Dir object is to track the protocol to be used
to transfer the directory content as well as to give the appropriate path depending
on where (client-side, remote-side, intermediary-side) the Dir.path is
being called from.
"""
import os
...
class Dir:
    ...
    def __init__(self, url: Union[os.PathLike, str]):
        """Construct a Dir object from a url string.
    ...
    def cleancopy(self) -> "File":
        """Returns a copy of the Dir containing only the global immutable state,
           without any mutable site-local local_path information. The returned Dir
